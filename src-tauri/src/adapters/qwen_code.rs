...
impl AppAdapter for QwenCodeAdapter {
    ...
    fn detect_sources(&self, ctx: &PlatformContext) -> Vec<(String, u32)> {
        vec![
            (
                ctx.workspace_file(".qwen/settings.json")
                    .to_string_lossy()
                    .to_string(),
                10,
            ),
...
mod tests {
    ...
    #[test]
    fn parses_qwen_settings() {
        let parsed = QwenCodeAdapter.parse_source(
            &ctx(),
    ...
    fn plans_qwen_merge() {
        ...
        apps.insert(SupportedApp::QwenCode, true);
        let op = QwenCodeAdapter.plan_apply(
            &ctx(),
            &MCPConfig {
                version: 1,
