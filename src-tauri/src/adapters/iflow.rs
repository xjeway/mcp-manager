...
impl AppAdapter for IFlowAdapter {
    ...
    fn detect_sources(&self, ctx: &PlatformContext) -> Vec<(String, u32)> {
        vec![
            (
                ctx.workspace_file(".iflow/settings.json")
                    .to_string_lossy()
                    .to_string(),
                10,
            ),
...
mod tests {
    ...
    #[test]
    fn parses_iflow_settings() {
        let parsed = IFlowAdapter.parse_source(
            &ctx(),
            "/Users/test/.iflow/settings.json",
            20,
            r#"{"language":"en","mcpServers":{"filesystem":{"command":"npx","args":["-y","@modelcontextprotocol/server-filesystem"]}}}"#,
        );
